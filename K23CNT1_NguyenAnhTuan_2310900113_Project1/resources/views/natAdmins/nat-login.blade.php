...
<body>
    ...
    <div class="wrap">
        <h1>Login</h1>
        <form action="{{ route('admins.natLoginSubmit') }}" method="POST">
            @csrf

            <div class="user-box">
                <input type="text" name="natTaiKhoan" id="natTaiKhoan" value="{{ old('natTaiKhoan') }}" />
                <label for="natTaiKhoan">Username</label>
                @error('natTaiKhoan')
            ...
            <div class="user-box">
                <input type="password" name="natMatKhau" id="natMatKhau" />
                <label for="natMatKhau">Password</label>
                @error('natMatKhau')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
    ...
    <script>
      ...
      function toggleForgetPassword() {
        ...
        if (isChecked) {
          const username = document.getElementById('natTaiKhoan').value;
          const password = document.getElementById('natMatKhau').value;
          document.cookie = `natTaiKhoan=${username}; path=/; max-age=31536000`; // 1 year
          document.cookie = `natMatKhau=${password}; path=/; max-age=31536000`; // 1 year
        } else {
          // Remove cookies when "Remember me" is unchecked
          document.cookie = 'natTaiKhoan=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
          document.cookie = 'natMatKhau=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
        }
      }
      ...
      window.onload = function() {
        ...
        cookies.forEach(cookie => {
          if (cookie.trim().startsWith('natTaiKhoan=')) {
            username = cookie.trim().split('=')[1];
          }
          if (cookie.trim().startsWith('natMatKhau=')) {
            password = cookie.trim().split('=')[1];
          }
        });

        if (username && password) {
          document.getElementById('natTaiKhoan').value = username;
          document.getElementById('natMatKhau').value = password;
